...
The MGF of a $d$-dimensional r.v $X\sim P$ is defined as $M^X(u)=\mathbb{E}[e^{u'X}]$ for $u\im\mathbb{R}^d$. We can use it to calculate moments of $X$ by evaluating its derivatives at 0.
...
## Bayes
- Densities: Prior:$\pi(\thea)$; Lieklihood: $p(x\mid\theta)$; Joint density $p(\theta, x)=\pi(\theta)p(x\mid\theta)$
